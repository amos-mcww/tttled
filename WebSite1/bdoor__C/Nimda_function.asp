...
If request.QueryString("Class")="big" Then '一级栏目添加
    ...
    If Not (Rs.Eof and Rs.Bof) or request.Form("big_name")="" or request.Form("big_wj_name")="" Then
        response.write "<script language='javascript'>alert('[该一级分类已存在]或[一级栏目名为空]或[静态文件链接名称为空]，请更正');history.go(-1);</script>"
    Else
        Rs.Addnew
        ...
        Rs.update
        Response.Redirect "Nimda_class.asp?Lei=添加成功&Edit=B_Z"
        RS.close
	End If

elseif request.QueryString("Class")="big_Edit" Then '一级栏目修改
    Set Rs=server.createobject("ADODB.Recordset")
...
elseif request.QueryString("Class")="big_Del" Then'一级栏目删除
...
elseif request.QueryString("Class")="small" Then '二级栏目添加
    Set Rs=server.createobject("ADODB.Recordset")
	...
	If Not (Rs.Eof and Rs.Bof) or request.Form("small_name")="" or request.form("big_name")="" or request.form("small_wj_name")="" Then
		response.write "<script language='javascript'>alert('[该二级分类已存在]或[二级栏目名为空]或[静态文件链接名称为空]，请更正');history.go(-1);</script>"
	Else
        Rs.Addnew
        ...
        if request.Form("clkj_tj")="yes" then
            Rs("clkj_tj")=1
        else
            Rs("clkj_tj")=0
        End If
        Rs.update
        Response.Redirect "Nimda_class.asp?Lei=二级栏目添加成功&Edit=S_Z"
        RS.close
        Rss.close
    End If

elseif request.QueryString("Class")="small_Edit" Then '二级栏目修改
    Set Rs=server.createobject("ADODB.Recordset")
...
elseif request.QueryString("Class")="small_Del" Then'二级栏目删除
...
elseif request.QueryString("Class")="meun" Then '一级栏目添加
    Set Rs=server.createobject("ADODB.Recordset")
    ...
    If Not (Rs.Eof and Rs.Bof) or request.Form("meun_name")="" Then
        response.write "<script language='javascript'>alert('重复添加或栏目名为空，请更正');history.go(-1);</script>"
    Else
        Rs.Addnew
        ...
        Rs.update
        Response.Redirect "Nimda_menu.asp?Lei=栏目添加成功"
        RS.close
    End If

elseif request.QueryString("Class")="meun_edit" Then '栏目修改
    Set Rs=server.createobject("ADODB.Recordset")
...
elseif request.QueryString("Class")="meun_Del" Then'栏目删除
...
'============================== 参数设置 ===============================================================================
elseif Request.Querystring("Class")="cansu" and trim(Request.Form("clkj_name"))<>"" and trim(request.form("index_key"))<>"" and Request.Form("jiaodu")<>"" Then
	Set Rs=server.createobject("ADODB.Recordset")
	...
	Rs("clkj_config_gallery_sltk") = Request.Form("clkj_gallery_w")
...
'============================== 友情链接 ===============================================================================
elseif Request.Querystring("Class")="links" Then
	Set Rs=server.createobject("ADODB.Recordset")
    ...
    If Not (Rs.Eof and Rs.Bof) or request.Form("clkj_name")="" Then
        response.write "<script language='javascript'>alert('重复添加或链接名为空，请更正');history.go(-1);</script>"
	Else
    	Rs.Addnew
    	...
    	Rs.update
    	Response.Redirect "Nimda_links.asp?Lei=添加改成功"
    	Rs.close
  End If

elseif Request.Querystring("Class")="links_Edit" Then
	Set Rs=server.createobject("ADODB.Recordset")
...
elseif Request.Querystring("Class")="pas" Then
    ...
    If Not (Rs.Eof and Rs.Bof) or request.Form("user_name")="" Then
        response.write "<script language='javascript'>alert('重复添加或用户名为空，请更正');history.go(-1);</script>"
	Else
    	Rs.Addnew
    	...
    	Rs.update
    	Response.Redirect "Nimda_user.asp?Lei=添加成功"
    	Rs.close
   End if

elseif Request.Querystring("Class")="pas_Edit" Then
	Set Rs=server.createobject("ADODB.Recordset")
...
elseif request.QueryString("Class")="pas_Del" Then'栏目删除
...
elseif Request.Querystring("Class")="content" Then
    Set Rs=server.createobject("ADODB.Recordset")
    Sql="select * from clkj_News"
    Rs.open Sql,conn,1,3
    If request.Form("Lei")="" Then
        response.write "<script language='javascript'>alert('新闻类别不能为空，请更正');history.go(-1);</script>"
    Else
        Rs.Addnew
...
elseif Request.Querystring("Class")="content_Edit" Then
    Set Rs=server.createobject("ADODB.Recordset")
...
elseif Request.Querystring("Class")="news_Del" Then
...
elseif Request.Querystring("Class")="Product" Then
	...
	IF request.Form("big_Lei")="" Then
        response.write "<script language='javascript'>alert('注意：产品一级类别,产品文件名不能为空');history.go(-1);</script>"
	Else
        Rs.Addnew
...
'图片删除
elseif Request.Querystring("Class")="P_Del" Then
...
'栏目
elseif Request.Querystring("Class")="Moban" Then
    Set Rs=server.createobject("ADODB.Recordset")
    Sql="select * from clkj_config where clkj_config_id=1"
    Rs.open Sql,conn,1,3
...
'后台进入
elseif Request.Querystring("Class")="userpas" Then
    ...
    IF Rs.eof or Rs.bof then
        response.write "<script language='javascript'>alert('用户名或密码错误!');top.location.href='!Index.html';</script>"
        rs.close
        set rs=nothing
        response.end()
    Else
        ...
        response.cookies("username").expires=now()+2
        'session.Timeout=60
    End IF
...
'安全退出
elseif request.querystring("class")="Out" Then
	session("username") = ""
	session("userpas") = ""
...
elseif Request.Querystring("Class")="keyy" Then
	Set Rs=server.createobject("ADODB.Recordset")
	Sql="select * from key where keyname='"&trim(request.Form("key_name"))&"'"
	Rs.open Sql,conn,1,3
    If Not (Rs.Eof and Rs.Bof) or request.Form("key_name")="" Then
        response.write "<script language='javascript'>alert('该关键字已存在或为空，请更正');history.go(-1);</script>"
	Else
    	Rs.Addnew
    	...
    	Rs.update
    	Response.Redirect "Nimda_key.asp?Lei=添加改成功"
    	Rs.close
    end if

elseif Request.Querystring("Class")="key_Edit" Then
	Set Rs=server.createobject("ADODB.Recordset")
...
elseif request.QueryString("Class")="key_Del" Then'栏目删除
...
'''''''''''''''''''''''''''''''' 一级分类 '''''''''''''''''''''''''''''''''''''''''''''''''''
'1.1: 修改一级分类
elseif request.QueryString("Class")="gallery_big_edit" Then
    Set Rs=server.createobject("ADODB.Recordset")
    ...
    '修改clkj_gallery_SmallClass表 二级分类下相对应的一级分类名称
    Set Rs_s=server.createobject("ADODB.Recordset")
    ...
    '修改gallery 内容clkj_gallery表中的一级分类名称
    Set Rs_p=server.createobject("ADODB.Recordset")
    ...
    Response.Redirect "Nimda_gallery_sort.asp"
    Rs.close

' 1.2: 添加一级分类
elseif request.QueryString("Class")="gallery_big_add" Then
    Set Rs=server.createobject("ADODB.Recordset")
    Sql="select * from clkj_gallery_BigClass where clkj_BigClassName = '"&trim(request.Form("big_name"))&"'"
    Rs.open Sql,conn,1,3
    If Not (Rs.Eof and Rs.Bof) or request.Form("big_name")="" Then
        response.write "<script language='javascript'>alert('[该一级分类已存在]或[一级分类名为空]，请更正');history.go(-1);</script>"
    Else
        Rs.Addnew
        Rs("clkj_BigClassName") = trim(request.Form("big_name"))
        RS("clkj_BigClassurl") = trim(request.Form("big_name"))
        Rs("clkj_BigClasskey") = request.Form("big_key_name")
        Rs("clkj_BigClassdes") = request.Form("big_ms_name")
        Rs("big_paixu") = request.Form("big_paixu")
        Rs.update
        Response.Redirect "Nimda_gallery_class.asp?Lei=添加成功&Edit=gallery_big_add"
        RS.close
    End If

' 1.3: 删除一级分类
elseif request.QueryString("Class")="gallery_big_class_del" Then
    Sql="delete * from clkj_gallery_BigClass where clkj_BigClassID = "&request("clkj_BigClassID")
    conn.execute Sql
    Sql_1="delete * from clkj_gallery_SmallClass where clkj_BigClassID = "&request("clkj_BigClassID")
    conn.execute Sql_1
    Set Rs_d=server.createobject("ADODB.Recordset")
    Sql_del="select * from clkj_gallery where clkj_BigClassID = "&request("clkj_BigClassID")
    Rs_d.open Sql_del,conn,1,1

    do while not Rs_d.eof
        Call gallery_DelFenLeiImages(Rs_d("clkj_prpic"))
        Rs_d.Movenext
    Loop
    Rs_d.close

    Sql_s="delete * from clkj_gallery where clkj_BigClassID = "&request("clkj_BigClassID")
    conn.execute Sql_s

    Response.Redirect "Nimda_gallery_sort.asp?Lei=一级分类删除成功"

' 2.1: 修改二级分类
elseif request.QueryString("Class")="gallery_small_edit" Then
    Set Rs=server.createobject("ADODB.Recordset")
    ...
    '读出大类别名称，写入小类别表中开始
    Set Rss=server.createobject("ADODB.Recordset")
    Sqls="select * from clkj_gallery_BigClass where clkj_BigClassID ="&trim(request.Form("big_name"))
    Rss.open Sqls,conn,1,1
    big_name=Rss("clkj_BigClassName")
    '读出大类别名称，写入小类别表中结束
    Rs("clkj_BigClassName") = big_name
    Rs("clkj_smallClassName") = trim(request.Form("small_name"))
    RS("clkj_smallClassurl") = trim(request.Form("small_name"))
    Rs("clkj_smallClasskey") = request.Form("small_key_name")
    Rs("clkj_smallClassdes") = request.Form("small_ms_name")
    Rs("small_paixu") = request.Form("small_paixu")
    if request.Form("clkj_tj")="yes" then
    ...
    '修改二级栏目时相对应的修改gallery中相对应的类别
    Set Rs_sp=server.createobject("ADODB.Recordset")
    Sql_sp="select * from clkj_gallery where clkj_SmallClassID = "&request("clkj_SmallClassID")
    Rs_sp.open Sql_sp,conn,1,3
    if not rs_sp.eof then
        do while not rs_sp.eof
             Rs_sp("clkj_SmallClassName") = trim(request.Form("small_name"))
    ...
    Rs_sp.close
    ''Response.Redirect "Nimda_gallery_class.asp?Lei=二级分类修改成功&Edit=gallery_small_edit&clkj_SmallClassID="&request("clkj_SmallClassID")
    Response.Redirect "Nimda_gallery_sort.asp"
    Rs.close

' 2.2: 添加二级分类
elseif request.QueryString("Class")="gallery_small_add" Then
    Set Rs=server.createobject("ADODB.Recordset")
    Sql="select * from clkj_gallery_SmallClass where clkj_SmallClassName = '"&trim(request.Form("small_name"))&"'"
    Rs.open Sql,conn,1,3
    '判断是否有重名
    If Not (Rs.Eof and Rs.Bof) or request.Form("small_name")="" or request.form("big_name")="" Then
        response.write "<script language='javascript'>alert('[该二级分类已存在]或[二级栏目名为空]或[该一级分类为空]，请更正');history.go(-1);</script>"
    Else
        Rs.Addnew
        Rs("clkj_BigClassID") = trim(request.Form("big_name"))
        '读出大类别名称，写入小类别表中开始
        ...
        End If
        Rs.update
        Response.Redirect "Nimda_gallery_class.asp?Lei=二级分类添加成功&Edit=small_class_add"&request("clkj_SmallClassID")
        RS.close
        Rss.close
    End If

' 2.3: 删除二级分类
elseif request.QueryString("Class")="gallery_small_class_del" Then
    Sql="delete * from clkj_gallery_SmallClass where clkj_SmallClassID = "&request("clkj_SmallClassID")
    conn.execute Sql
    Set Rs_d=server.createobject("ADODB.Recordset")
    Sql_del="select * from clkj_gallery where clkj_SmallClassID = "&request("clkj_SmallClassID")
    Rs_d.open Sql_del,conn,1,1
    do while not Rs_d.eof
        Call gallery_DelFenLeiImages(Rs_d("clkj_prpic"))
        Rs_d.Movenext
    Loop
    Rs_d.close
    Sql_s="delete * from clkj_gallery where clkj_SmallClassID = "&request("clkj_SmallClassID")
    conn.execute Sql_s
    Response.Redirect "Nimda_gallery_sort.asp?Lei=二级删除成功"

'''''''''''''''''''''''''''''''' 一级分类 END '''''''''''''''''''''''''''''''''''''''''''''''''''
';;;;;;;;;;;;;;;;;;;;;;;;;;;;; 5. gallery 管理 ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
' gallery 添加 '
' 添加 gallery 内容'
elseif Request.Querystring("Class")="gallery_add" Then
    Set Rs=server.createobject("ADODB.Recordset")
    Sql="select * from clkj_gallery"
    Rs.open Sql,conn,1,3
    IF request.Form("big_Lei")="" Then
        response.write "<script language='javascript'>alert('注意：一级类别不能为空');history.go(-1);</script>"
    Else
        Rs.Addnew
        Rs("clkj_BigClassID") = Request.Form("big_Lei")
        '读出大类别名称，写入表中开始
        Set Rss=server.createobject("ADODB.Recordset")
        Sqls="select * from clkj_gallery_BigClass where clkj_BigClassID ="&trim(request.Form("big_Lei"))
        Rss.open Sqls,conn,1,1
        if not Rss.eof then
            bigg_name=Rss("clkj_BigClassName")
            bigll_ml=Rss("clkj_BigClassurl")
      ...
      '读出大类别名称，写入表中结束
        ...
        if  isNumeric(Request.Form("small_Lei"))=true then
            ...
            '读出小类别名称，写入表中开始
            Set Rsss=server.createobject("ADODB.Recordset")
            Sqlss="select * from clkj_gallery_SmallClass where clkj_SmallClassID ="&trim(request.Form("small_Lei"))
            Rsss.open Sqlss,conn,1,1
            smalll_name=Rsss("clkj_SmallClassName")
            smalll_ml=Rsss("clkj_SmallClassurl")'小类换大类链接地址
            Rsss.close
        ...
        if isobjinstalled("Persits.Jpeg") Then
            ...
            For Each PStrss In ProductsPic
                ...
                IF PStrss<>" " and PStrss<>""  Then
                    Call gallery_TradeCmsJpeg(Imgop,PStrss,ImgPic,ImgText,gallery_ImgWidth,Imgyanse,Imgjiaodu)
                End IF
            next
        end if

        'call shengchen(tt)添加生成静态
        Response.Redirect "Nimda_gallerys.asp"
        Rs.close
    End IF
' 5.1 gallery 删除
elseif Request.Querystring("Class")="gallery_del" Then
    delid=Split(request("delid"),",")'图片删除
    For Each Strss In delid
...
' 5.2 gallery 修改
elseif Request.Querystring("Class")="gallery_edit" Then
	Set Rs=server.createobject("ADODB.Recordset")
    if request.Form("lscp")="yes" then'发布类似产品
        Sql="select * from clkj_gallery"
    else
        Sql="select * from clkj_gallery where clkj_prid="&request("clkj_prid")
    end if
    Rs.open Sql,conn,1,3
	IF request.Form("big_Lei")="" Then
        response.write "<script language='javascript'>alert('注意：一级类别名不能为空');history.go(-1);</script>"
	elseif request.Form("lscp")="yes" then'发布类似产品
	    Rs.Addnew
	end if
    Rs("clkj_BigClassID") = request("clkj_BigClassID")
    '读出大类别名称，写入表中开始
    Set Rss=server.createobject("ADODB.Recordset")
    Sqls="select * from clkj_gallery_BigClass where clkj_BigClassID="&trim(request("clkj_BigClassID"))
    Rss.open Sqls,conn,1,1
    if not Rss.eof then
        bigg_name=Rss("clkj_BigClassName")
        bigll_ml=Rss("clkj_BigClassName")
    End if
    Rss.close
    '读出大类别名称，写入表中结束
    if  isNumeric(Request.Form("small_Lei"))=true then
        ...
        Rsss.open Sqlss,conn,1,1
        smalll_name=Rsss("clkj_SmallClassName")
        smalll_ml=Rsss("clkj_BigClassName")
        Rsss.close
    end if
    '读出小类别名称，写入表中结束
    ...
    IF isobjinstalled("Persits.Jpeg") Then
        ...
        For Each XStrss In ProductsPicx
            IF trim(XStrss)<>" " and trim(XStrss)<>"" Then'图片不为空执行
                if instr(xStrss,"../") <> 0 then
                    Call gallery_TradeCmsJpeg(Imgop,trim(xStrss),ImgPic,ImgText,gallery_ImgWidth,Imgyanse,Imgjiaodu)'图片打水印
                End IF
            End if
        next
    End IF
    ...
    if request.form("sf")="s" then
        Response.Redirect "nimda_gallerys.asp?clkj_BigClassID="&request("clkj_BigClassID")&"&clkj_BigClassName="&bigg_name&"&clkj_SmallClassID="&Request.form("small_Lei")&"&clkj_SmallClassName="&smalll_name&"&ToPage="&Request.form("ToPage")&"&sf=s"
    elseif request.form("sf")="b" then
        Response.Redirect "nimda_gallerys.asp?clkj_BigClassID="&request("clkj_BigClassID")&"&clkj_BigClassName="&bigg_name&"&ToPage="&Request.form("ToPage")&"&sf=b"
    else
        Response.Redirect "nimda_gallerys.asp?ToPage="&Request.form("ToPage")
    end if
    Rs.close
...
''''''' gallery '''''''''
Sub gallery_Big_input()'所有一级分类，表单形式
    Set Rs=server.createobject("ADODB.Recordset")
...
Sub gallery_big_lie()'列出所有一级分类
	...
	Else
		Do while not Rs.Eof
		IF rs("clkj_BigClassID")=cint(request("clkj_BigClassID")) Then
		response.write "<a href=nimda_gallerys.asp?clkj_BigClassID="&rs("clkj_BigClassID")&"&clkj_BigClassName="&Server.UrlEncode(rs("clkj_BigClassName"))&"&sf=b><font color='#FF0000;'><b>"&Rs("clkj_BigClassName")&"</b></font></a>&nbsp;&nbsp;"
		Else
		response.write "<a href=nimda_gallerys.asp?clkj_BigClassID="&rs("clkj_BigClassID")&"&clkj_BigClassName="&Server.UrlEncode(rs("clkj_BigClassName"))&"&sf=b>"&Rs("clkj_BigClassName")&"</a>&nbsp;&nbsp;"
		End if
		Rs.Movenext
		Loop
		Rs.close
...
Sub gallery_small_lie()'列出所有二级分类
	Set Rs=server.createobject("ADODB.Recordset")
		Sql="select * from clkj_gallery_SmallClass where clkj_BigClassID="&cint(request("clkj_BigClassID"))
		Rs.open Sql,conn,1,1
	If Rs.Eof or Rs.Bof Then
		response.write "暂无内容"
	Else
		Do while not Rs.Eof
		IF rs("clkj_SmallClassID")=cint(request("clkj_SmallClassID"))Then
		response.write "<a href=nimda_gallerys.asp?clkj_SmallClassID="&Server.UrlEncode(rs("clkj_SmallClassID"))&"&clkj_BigClassID="&request("clkj_BigClassID")&"&clkj_BigClassName="&Server.UrlEncode(request("clkj_BigClassName"))&"&clkj_SmallClassName="&Server.UrlEncode(RS("clkj_SmallClassName"))&"&sf=s><font color='#FF0000;'><b>"&Rs("clkj_SmallClassName")&"</b></font></a>&nbsp;&nbsp;&nbsp;"
		Else
		response.write "<a href=nimda_gallerys.asp?clkj_SmallClassID="&Server.UrlEncode(rs("clkj_SmallClassID"))&"&clkj_BigClassID="&request("clkj_BigClassID")&"&clkj_BigClassName="&Server.UrlEncode(request("clkj_BigClassName"))&"&clkj_SmallClassName="&Server.UrlEncode(RS("clkj_SmallClassName"))&"&sf=s>"&Rs("clkj_SmallClassName")&"</a>&nbsp;&nbsp;&nbsp;"
		End if
		Rs.Movenext
		Loop
		Rs.close

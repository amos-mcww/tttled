...
IF isobjinstalled("Scripting.FileSystemObject") and isobjinstalled("adodb.stream") Then

	'判断此文件是否存在
	
	Set sFileObject=Server.CreateObject("Scripting.FileSystemObject")
	sfilepath=Server.MapPath("install.asp")
	if sFileObject.FileExists(sfilepath) then response.Redirect("install.asp")
	
Else

response.write "<script language='javascript'>alert('.^.你的系统不支持Fso组件\n或不技持adodb.stream组件\n请先按装fso组件adodb.stream组件再进行安装此系统');location.href='http://www.sem-cms.com/talk/?viewid=255';</script>"	
...
Public Function MD5(sMessage)
    ...
    'MD5 = LCase(WordToHex(a) & WordToHex(b) & WordToHex(c) & WordToHex(d))5
